...
class GoogleHttpClient {
   ...
   public function validate($verify_token = null, $user = null) {
      ...
      if ($verify_token && !isset($verify_token['user_id'])) {
         $arr[] = 'key \'user_id\' at argument $verify_token does not exist';
      }
      ...
      if ($verify_token && !isset($verify_token['issued_to'])) {
         $arr[] = 'key \'issued_to\' at argument $verify_token does not exist';
      }
      if(\count($arr)) {
   ...
   public function refresh($refresh_token = null) {
      ...
      $data = \json_decode($buffer, true);
      if ($http_code != 200) {
         $log = __CLASS__ . '::refresh() error: http code not 200. Responded: '.print_r($data, true);
         $return = false;
      } else {
         $this->auth = $data;
   ...
   public function revokeToken($access_token = null) {
      ...
      $data = \json_decode($buffer, true);
      if ($http_code != 200) {
         $log = __CLASS__ . '::revokeToken() error: http code not 200. Responded: '.print_r($data, true);
         $return = false;
      } else {
         $this->auth = null;
         $this->user = null;
   ...
   public function userInfo($access_token = null) {
      ...
      $data = \json_decode($buffer, true);      
      if ($http_code != 200) {
         $log = __CLASS__ . '::userInfo() error: http code not 200. Responded: '.print_r($data, true);
         $return = false;
      } else {
         $this->user = $data;
